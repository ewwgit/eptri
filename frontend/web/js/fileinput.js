...
(function ($) {
    ...
    tCaption = '<div tabindex="100" class="form-control file-caption {class}">\n' +
        '   <div class="file-caption-name"></div>\n' +
        '</div>\n',
    tBtnDefault = '<button type="{type}" tabindex="100" title="{title}" class="{css}"{status}>{icon}{label}</button>',
    tBtnLink = '<a href="{href}" tabindex="100" title="{title}" class="{css}"{status}>{icon}{label}</a>',
    tBtnBrowse = '<div tabindex="100" class="{css}"{status} ">{icon}{label}</div>',
